...
class QuadrupleOptionElement(name: String) : OptionElement<Int>(name) {
    override fun readValue(x: Int, y: Int): Int {
        val valueX = x + width - (QUADRUPLE_VALUE_WIDTH * 4) + 1

        val capture = takeScreen().capture(valueX, y, QUADRUPLE_VALUE_WIDTH * 4, height)
        val color0 = Color(capture.getRGB(1, 1), true)
        val color1 = Color(capture.getRGB(QUADRUPLE_VALUE_WIDTH + 1, 1), true)
        val color2 = Color(capture.getRGB((QUADRUPLE_VALUE_WIDTH * 2) + 1, 1), true)
        val color3 = Color(capture.getRGB((QUADRUPLE_VALUE_WIDTH * 3) + 1, 1), true)

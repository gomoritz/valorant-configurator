        ...
        try {
            ...
            if (value != null) {
                when (element) {
                    ...
                    is SingleKeybindOptionElement -> element.writeValue(x, y, value.takeIf { !it.isNull }?.textValue())
                    is FieldOptionElement -> element.writeValue(x, y, value.doubleValue())
                    is UnlabelledSliderOptionElement -> element.writeValue(x, y, value.doubleValue())
                    is CheckboxElement -> element.writeValue(x, y, value.booleanValue())
                }
            } else {
                Logger.info("Skipping setting <${element.name}>", "SettingsWriter")
            }
        } catch (e: Exception) {
